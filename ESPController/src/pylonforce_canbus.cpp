...
void pylonforce_message_7320()
{
  ...
  data.voltage_level = mysettings.cellmaxmv * mysettings.totalNumberOfSeriesModules / 1000;
...
{
//  ESP_LOGV(TAG, "pylonforce_message_7330_7340: hostname=")
...
//  const char text[8] = "BLUBB\0\0";
//send_ext_canbus_message(0x7330+mysettings.canbus_equipment_addr, (uint8_t *)&text, 8);
//send_ext_canbus_message(0x7340+mysettings.canbus_equipment_addr, (uint8_t *)&text, 8);
  const char text1[8] = {'P','Y','L','O','N','T','E','C'};
  const char text2[8] = {'H', 0, 0, 0, 0, 0, 0, 0};
send_ext_canbus_message(0x7330+mysettings.canbus_equipment_addr, (uint8_t *)&text1, 8);
send_ext_canbus_message(0x7340+mysettings.canbus_equipment_addr, (uint8_t *)&text2, 8);
...
void pylonforce_message_4220()
{
  struct data4220
  {
    ...
    uint16_t battery_discharge_current_limit; // Max discharge current, resolution 0.1A, offset -3000A (therefore logically <=30000, as discharge current is negative Amps)
  };
...
void pylonforce_message_4240()
{
  ...
  {
    data.max_single_battery_cell_temperature = -100;  // 0°C
    data.min_single_battery_cell_temperature = -100;  // 0°C
...
void pylonforce_message_4250()
{
  struct data4250
  {
    ...
    uint8_t error_other: 1; // Other error
    ...
    uint16_t alarm_reserve15: 1;
    uint16_t alarm_reserve14: 1;
    uint16_t alarm_reserve13: 1;
    uint16_t alarm_reserve12: 1;
    uint16_t alarm_mhv: 1; // module high voltage alarm
    uint16_t alarm_mlv: 1; // module low voltage alarm
    uint16_t alarm_doca: 1; // discharge over current alarm
    uint16_t alarm_coca: 1; // charge over current alarm
    uint16_t alarm_dht: 1; // discharge cell high temperature alarm
    uint16_t alarm_dlt: 1; // discharge cell low temperature alarm
    uint16_t alarm_cht: 1; // charge cell high temperature alarm
    uint16_t alarm_clt: 1; // charge cell low temperature alarm
    uint16_t alarm_phv: 1; // charge system high voltage alarm
    uint16_t alarm_plv: 1; // charge system low voltage alarm
    uint16_t alarm_bhv: 1; // single cell high voltage alarm
    uint16_t alarm_blv: 1; // single cell low voltage alarm
    ...
    uint16_t protect_reserve15: 1;
    uint16_t protect_reserve14: 1;
    uint16_t protect_reserve13: 1;
    uint16_t protect_reserve12: 1;
    uint16_t protect_mhv: 1; // module high voltage protect
    uint16_t protect_mlv: 1; // module low voltage protect
    uint16_t protect_doca: 1; // discharge over current protect
    uint16_t protect_coca: 1; // charge over current protect
    uint16_t protect_dht: 1; // discharge cell high temperature protect
    uint16_t protect_dlt: 1; // discharge cell low temperature protect
    uint16_t protect_cht: 1; // charge cell high temperature protect
    uint16_t protect_clt: 1; // charge cell low temperature protect
    uint16_t protect_phv: 1; // charge system high voltage protect
    uint16_t protect_plv: 1; // charge system low voltage protect
    uint16_t protect_bhv: 1; // single cell high voltage protect
    uint16_t protect_blv: 1; // single cell low voltage protect
  };
  ...
  if (_controller_state == ControllerState::Running)
  {
    ...
    data.alarm_mhv = ((rules.rule_outcome[Rule::BankOverVoltage] || rules.rule_outcome[Rule::CurrentMonitorOverVoltage]) ? 1 : 0);
    data.alarm_mlv = ((rules.rule_outcome[Rule::BankUnderVoltage] || rules.rule_outcome[Rule::CurrentMonitorUnderVoltage]) ? 1 : 0);

    // TODO: maybe calculate from dynamic charge current?
    data.alarm_coca = rules.rule_outcome[Rule::CurrentMonitorOverCurrentAmps] ? 1 : 0;
    data.alarm_doca = rules.rule_outcome[Rule::CurrentMonitorOverCurrentAmps] ? 1 : 0;

    data.alarm_bhv = ((rules.rule_outcome[Rule::ModuleOverVoltage]) ? 1 : 0);
    data.alarm_blv = ((rules.rule_outcome[Rule::ModuleUnderVoltage]) ? 1 : 0);

    if (rules.moduleHasExternalTempSensor)
    {
      data.alarm_cht = (rules.rule_outcome[Rule::ModuleOverTemperatureExternal] ? 1 : 0);
      data.alarm_clt = (rules.rule_outcome[Rule::ModuleUnderTemperatureExternal] ? 1 : 0);
    }
...
//  data.error_in_comm = ((rules.rule_outcome[Rule::BMSError] | rules.rule_outcome[Rule::EmergencyStop]) ? 1 : 0);
  data.error_other = ((_controller_state != ControllerState::Running || 
      rules.rule_outcome[Rule::BMSError] || 
      rules.rule_outcome[Rule::EmergencyStop]) ? 1 : 0);
//  data.error_other = 1;
//  data.protect_plv = 1;
//  data.alarm_cht = 1;
//  data.protect_cht = 1;
...
In this 5 minutes, if there is a protection alarm, BMS will cut off the relay as normal
    ...
    // reply: OK, will act this command immediately
    //data[0] = 0xAA;
