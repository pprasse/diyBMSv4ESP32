...
#else
  // Used for temperature readings only (11 bit ADC with oversample)
  #define MAXIUMUM_ATTINY_ADC_SCALE 2047.0F
#endif
...
public:
  ...
  {
    #if !defined(__AVR_ATtinyx14__)   // 1614 will not wake up. Seems to be related to errata "TXD Pin Override Not Released When Disabling the Transmitter"
  ...
  {
    #if !defined(__AVR_ATtinyx14__)   // 1614 tx was not disabled
  ...
  static void SelectCellVoltageChannel()
  {
      ...
      ADC0.CTRLB = ADC_SAMPNUM_enum::ADC_SAMPNUM_ACC64_gc;
    #else
  ...
  static inline void SelectInternalTemperatureChannel()
  {
    // PA7
    #if defined(__AVR_ATtinyx14__)
      ADC0.CTRLB = ADC_SAMPNUM_enum::ADC_SAMPNUM_ACC2_gc;
    #else
      // FREERUN / LEFTADJ / SAMPNUM[3:0]
